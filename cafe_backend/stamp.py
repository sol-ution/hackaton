...
- 하루 최대 3개까지
- 매장별 목표 칸수를 채우면 쿠폰 자동 발급 후 스탬프 리셋
...
def load_stamps(user_id: int = DEMO_USER_ID) -> list[dict]:
    return [s for s in _read(STAMPS_CSV) if int(s["userId"]) == user_id]


def load_coupons(user_id: int = DEMO_USER_ID) -> list[dict]:
    return [c for c in _read(COUPONS_CSV) if int(c["userId"]) == user_id]
...
def stamp_count(cafe_id: int, user_id: int = DEMO_USER_ID) -> int:
    return sum(1 for s in load_stamps(user_id)
...
def earned_today(user_id: int = DEMO_USER_ID) -> int:
    ...
    return sum(1 for s in load_stamps(user_id)
               if s["earned"] == "true"
               and datetime.fromisoformat(s["reportedAt"]) >= midnight)
...
def use_coupon(code: str, pin: str, cafe_row: dict,
               user_id: int = DEMO_USER_ID) -> dict:
    """쿠폰 사용 처리. 유저 폰에서 사장님이 매장 PIN을 입력한다."""
    with _lock:
